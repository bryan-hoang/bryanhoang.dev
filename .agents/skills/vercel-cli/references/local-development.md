...
1. **Link your project** — `vercel link` or `vercel link --repo` (monorepo).
   Check your team first with `vercel whoami`.
...
- `vercel link` — connect to a Vercel project. Use `--repo` for multi-project
  monorepos.
...
Run `vercel dev` from a project subdirectory (e.g., `apps/web/`) to skip the
