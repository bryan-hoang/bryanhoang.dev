...
**Use `vercel curl` to access preview deploys.** It handles deployment
protection automatically — no need to disable protection or manage bypass
secrets.
...
`vercel bisect` performs a binary search across deployments to find which one
